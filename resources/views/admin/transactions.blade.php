...
<div class="glass-panel border border-[#D4AF37]/20 p-6 rounded-2xl shadow-lg gold-glow">
    ...
    <div class="flex flex-col sm:flex-row gap-4 mb-6">
        <input type="text" id="tx-search" oninput="filterTransactions()" placeholder="Cari nama, invoice, email, atau whatsapp..." class="flex-1 px-4 py-3 rounded-xl bg-white border border-[#D4AF37]/30 focus:outline-none focus:border-[#D4AF37] text-xs text-slate-700 font-medium shadow-sm">
        
        <select id="tx-status-filter" onchange="filterTransactions()" class="px-4 py-3 rounded-xl bg-white border border-[#D4AF37]/30 focus:outline-none focus:border-[#D4AF37] text-xs text-slate-500 font-medium cursor-pointer shadow-sm">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody id="tx-table-body" class="text-xs divide-y divide-slate-50">
                ...
                @endphp
                <tr class="hover:bg-[#D4AF37]/5 transition-all tx-row" 
                    ...
                    <td class="py-3.5 px-4">
                        ...
                        <span class="block text-[9px] text-[#D4AF37] font-bold uppercase mt-0.5">Nomor: {{ $tx->candidate->candidate_number }} | Kategori: {{ strtoupper($tx->candidate->gender) }}</span>
                    </td>
                    <td class="py-3.5 px-4 text-center font-extrabold text-[#D4AF37] text-sm">{{ number_format($tx->vote_amount) }}</td>
                    ...
                    <td class="py-3.5 px-4 text-center">
                        ...
                        <a href="{{ route('vote.receipt', $tx->id) }}" target="_blank" class="px-2.5 py-1 rounded bg-[#D4AF37]/5 border border-[#D4AF37]/30 text-[#D4AF37] hover:bg-[#D4AF37]/15 transition-all font-bold text-[9px] uppercase tracking-wide">
                            Lihat
                        </a>
                        @else
